...
- **The Searcher (The Retriever):** The search engine (`DenseRetriever`). Did it bring the wrong document pages from the filing cabinet?
- **The Writer (The Generator / LLM, specifically `gemini-2.5-flash`):** The AI model that writes the answer (**NOT the human user**). Did the search engine bring the right page, but the AI language model misread, skipped, or failed to summarize a crucial detail?
...
1. **Test A (The Gold Test — Perfect Search):**  
   We completely bypassed the search engine. We handed the AI writer (`gemini-2.5-flash`) the **exact, perfect policy document** from the library. This measures the absolute best score our AI writer can ever achieve under perfect information (**The Generation Ceiling**).
2. **Test B (The Real System):**  
   We ran our real pipeline, where the AI writer answered using whatever documents our search engine found.

Then we did simple subtraction:
- **Retrieval Loss ($A - B$):** Points lost because the search engine missed a document.
- **Generation Loss ($1 - A$):** Points lost because the AI writer (`gemini-2.5-flash`) got confused or missed a detail, *even with the perfect document in its hands!*
...
- The bigger problem is the **AI writer (`gemini-2.5-flash`)**: even when given the 100% perfect document, the language model still missed **11.9 percentage points** of credit!
...
|---|---|---|---|
| **Q04** | Waiting period for pre-existing diseases | Stated the 36-month rule, but forgot to mention the rider that reduces it to 12 or 24 months. | **AI Model (`gemini-2.5-flash`) omitted a secondary clause.** |
| **Q05** | Grace period rules | Stated the 30-day rule for annual plans, but omitted the 15-day rule for monthly instalment plans. | **AI Model cut it for brevity (2-sentence limit).** |
| **Q10** | Grievance escalation | Mentioned the Insurance Ombudsman, but forgot to mention the 1-year deadline to file. | **AI Model missed a time limit.** |
...
| **Q19** | Silver room rent proportionate deduction | Calculated the 0.667 deduction correctly, but forgot to state that medicines/consumables are exempt. | **AI Model missed an exemption rule.** |
| **Q20** | Adding a 63-year-old mother | Stated plan eligibility, but forgot the 10% co-pay that kicks in when a parent is over 60. | **AI Model missed an age condition.** |
| **Q23** | Lasik surgery coverage | Stated that refractive surgery is excluded, but struggled with the exact 7.5 dioptre exception. | **AI Model struggled with numbers.** |
...
| **Q34** | Organ donor expenses | Confirmed in-patient donor coverage, but omitted the exclusion for donor screening tests. | **AI Model missed a minor exclusion.** |
| **Q35** | Dental exclusions | Listed the accidental dental rule, but missed the optional teeth cleaning rider. | **AI Model missed an optional rider.** |

### The Pattern:
- **8 out of 10 errors** were caused by the **AI Model (`gemini-2.5-flash`)** omitting a condition, age rule, or rider when summarizing.
...
- **Gold-Context Decomposition (The "Blame Test"):**  
  A scientific diagnostic procedure. By running the system twice—once with the human-curated Gold Context, and once with the search engine's Retrieved Context—we mathematically isolate how many errors were caused by the **Searcher** (the Retriever) versus the **Writer** (the AI Generation Model / LLM, `gemini-2.5-flash`).
- **Generation Ceiling ($A = 0.881$ / 88.1%):**  
  The maximum possible correctness score our AI writer (`gemini-2.5-flash`) can achieve under ideal conditions (when handed the perfect documents directly, with zero search errors). It represents the absolute upper limit of the model's reading comprehension.
...
- **Generation-Attributable Loss ($1 - A = 0.119$ / 11.9%):**  
  The fraction of credit lost because the AI writer (`gemini-2.5-flash`, **NOT the user**) failed to extract, synthesize, or include all relevant conditions, *even when it was holding the 100% perfect document in its hands*.
